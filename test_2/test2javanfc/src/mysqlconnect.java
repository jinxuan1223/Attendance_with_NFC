...
public class mysqlconnect {
    ...
    public static ObservableList<attDetails> getAttData() {
        ...
        try {
            ...
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                list.add(new attDetails(Integer.parseInt(rs.getString("att_id")), rs.getString("clockin_date"), rs.getString("clockin_time"), rs.getString("clockout_date"), rs.getString("clockout_time"), Integer.parseInt(rs.getString("isLate")), Integer.parseInt(rs.getString("emp_id"))));
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, e);
        }
